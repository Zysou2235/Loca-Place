...
import { isTravelerLang, tt, type TravelerLang } from "@/lib/traveler-i18n";
...
export default async function BoxPage({
  ...
  searchParams: Promise<{ checkoutError?: string; lang?: string }>;
}) {
  const { qr_slug } = await params;
  const { checkoutError, lang: langParam } = await searchParams;
  const lang: TravelerLang = isTravelerLang(langParam) ? langParam : "fr";
    ...
    <main className="mx-auto flex max-w-2xl flex-col gap-6 px-5 py-10">
      ...
      <header className="border-b border-neutral-200 pb-5">
        <div className="flex items-start justify-between gap-3">
          ...
          />
          <LanguageSwitcher qrSlug={box.qrSlug} lang={lang} />
        </div>
        <h1 className="mt-1 text-2xl font-bold">{box.name}</h1>
        {box.location && (
          <p className="mt-1 text-sm text-neutral-500">{box.location}</p>
        )}
        <p className="mt-3 text-sm text-neutral-600">{tt("intro", lang)}</p>
      </header>

      {checkoutError && (
        <p className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">
          {tt(
            CHECKOUT_ERROR_KEYS[checkoutError as keyof typeof CHECKOUT_ERROR_KEYS] ??
              "errorPayment",
            lang
          )}
        </p>
      ...
      <EmailCaptureForm qrSlug={box.qrSlug} lang={lang} />
