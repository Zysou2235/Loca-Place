...
import { resendActivation, type ResendState } from "../auth-actions";
...
export function ResendActivationForm() {
  ...
  if (state.sent) {
    return (
      <p className="rounded-lg bg-green-50 px-4 py-3 text-sm text-green-700">
        Si un compte non activé existe pour cet email, un nouveau lien
        d&apos;activation vient d&apos;être envoyé. Pensez à vérifier vos spams.
      </p>
    );
  }
  ...
  return (
    <form action={action} className="flex flex-col gap-4">
      ...
      >
        {pending ? "Veuillez patienter…" : "Renvoyer le lien d'activation"}
      </button>
